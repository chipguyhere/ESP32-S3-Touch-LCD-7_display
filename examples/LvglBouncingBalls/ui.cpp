 ...
 * holds two sliders (1..20 balls, and a speed scale) plus a Round/Square toggle.
 * Each ball is a random color and re-launches to a random height every floor
 * bounce.  The toggle swaps circles for squares so you can see how much frame
 * time LVGL spends anti-aliasing the rounded corners.
 */
...
static bool     g_round      = true;  /* true = circles, false = squares      */
...
/* Apply the current shape to every object.  Circles cost LVGL extra per frame
 * (anti-aliased rounded-corner masking); squares are plain rect fills, so
 * toggling lets you see how much frame time the roundness is costing. */
static void apply_shape(void) {
    int radius = g_round ? LV_RADIUS_CIRCLE : 0;
    for (int i = 0; i < MAX_BALLS; i++)
        lv_obj_set_style_radius(g_balls[i].obj, radius, 0);
}

static void shape_event_cb(lv_event_t *e) {
    lv_obj_t *sw = (lv_obj_t *)lv_event_get_target(e);
    g_round = lv_obj_has_state(sw, LV_STATE_CHECKED);
    apply_shape();
}
...
static lv_obj_t *make_ball(lv_obj_t *parent) {
    ...
    lv_obj_set_style_radius(b, g_round ? LV_RADIUS_CIRCLE : 0, 0);
...
void ui_init(void) {
    ...
    /* --- control strip: two labeled sliders + a shape toggle on the right --- */
    int m       = 12;                                /* margin                 */
    int slider_h = 16;
    int row_gap  = 14;
    int sw_col_w = 72;                               /* right column for the toggle */
    int slider_w = W - 2 * m - sw_col_w;             /* sliders leave room for it  */
    int y        = g_floor_y + m;

    /* Shape toggle, in the reserved right column: on = round, off = square. */
    lv_obj_t *shape_sw = lv_switch_create(scr);
    lv_obj_set_size(shape_sw, 50, 26);
    lv_obj_set_pos(shape_sw, W - m - 50, g_floor_y + m + 6);
    if (g_round) lv_obj_add_state(shape_sw, LV_STATE_CHECKED);
    lv_obj_add_event_cb(shape_sw, shape_event_cb, LV_EVENT_VALUE_CHANGED, NULL);

    lv_obj_t *shape_label = lv_label_create(scr);
    lv_obj_set_style_text_color(shape_label, lv_color_white(), 0);
    lv_label_set_text(shape_label, "Round");
    lv_obj_align_to(shape_label, shape_sw, LV_ALIGN_OUT_BOTTOM_MID, 0, 4);
